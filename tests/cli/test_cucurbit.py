import pytest
from cucurbit.cli.cucurbit import cli
from cucurbit.cli.artifacts import ArtifactsBuilderException


@pytest.fixture
def artifacts_builder(mocker):
    mock_builder = mocker.patch("cucurbit.cli.cucurbit.ArtifactsBuilder")
    mock_ab = mocker.Mock()
...
def test_new_command(cli_runner, artifacts_builder):
    ...
    # ensure a pyproject.toml is created and it contains cucurbit as a dependency
    assert any(
        args[0] == "pyproject.toml" and "cucurbit" in args[1]
...
def test_new_command_error(cli_runner, artifacts_builder):
    ...
    result = cli_runner.invoke(cli, ["new", "test_app"])
    assert result.exit_code == 0
    assert "Error creating new cucurbit app: Test error" in result.output
