...
from cucurbit.exceptions import ProgrammerError
from cucurbit.discovery import discover_classes
...
class RoutesExtension:
    ...
    def init_app(self, app):
        ...
        self._internal_controllers = discover_classes('cucurbit.controllers', ActionController)
        routes = import_module(app.config['ROUTES']['LOCATION'])
        routes.register(self)
        if not self.has_path('/'):
            self.root(to='cucurbit/welcome#show')
        ...
        if not hasattr(app, "extensions"):
            app.extensions = {}
        app.extensions["cucurbit.routes"] = self
    ...
    def _get_controller_class(self, controller_name):
        if controller_name.startswith('cucurbit/'):
            controller_name = controller_name.replace('cucurbit/', '')
