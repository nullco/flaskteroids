...
Handling user input from forms is a common task in web applications. Cucurbit provides a cohesive set of tools to build forms, handle submissions securely, and prevent common vulnerabilities.

## Building Forms

Cucurbit provides a simple and powerful way to build forms using the `form_with` helper and the `Form` object. The `form_with` helper can be used with a model object to automatically generate the form's action and method, and it also includes the CSRF token automatically.
...
To prevent mass assignment vulnerabilities, Cucurbit uses a technique inspired by Rails' **Strong Parameters**. The `params` object allows you to whitelist which parameters are permitted in your controller actions.
...
Cucurbit automatically includes CSRF (Cross-Site Request Forgery) protection on all non-GET requests. A CSRF token is generated and must be included in your forms. The `form_with` helper does this automatically.
